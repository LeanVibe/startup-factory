...
def classify_provider_error(error: Exception) -> Tuple[ProviderErrorCategory, bool]:
    ...
    if (
        ...
        or getattr(error, "status", 0) == 429
    ):
        return (ProviderErrorCategory.RATE_LIMIT, True)
    if (
        "unauthorized" in msg
        or "forbidden" in msg
        or getattr(error, "status", 0) in (401, 403)
    ):
    ...
    def can_execute(self) -> bool:
        ...
        if self.state == CircuitBreakerState.CLOSED:
            ...
            if (
                ...
                self.success_count = 0
                logger.info(
                    f"Circuit breaker for {self.provider_name} transitioning to HALF_OPEN"
                )
                return True
            return False
        else:  # HALF_OPEN
    ...
    def record_failure(self):
        ...
        if self.state == CircuitBreakerState.CLOSED:
            if self.failure_count >= self.config.failure_threshold:
                ...
                logger.warning(
                    f"Circuit breaker for {self.provider_name} opened due to {self.failure_count} failures"
                )
            ...
            self.success_count = 0
            logger.warning(
                f"Circuit breaker for {self.provider_name} failed recovery test, back to OPEN"
            )
...
class ReliableAIProvider(AIProviderInterface):
    ...
    def __init__(self, config: ProviderConfig):
        ...
        self.circuit_breaker = CircuitBreaker(
            config.reliability.circuit_breaker, config.name
        )
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self.session is None or self.session.closed:
            timeout = aiohttp.ClientTimeout(
                total=self.config.reliability.circuit_breaker.timeout
            )
    ...
    async def _retry_with_backoff(self, func: Callable, *args, **kwargs) -> Any:
            ...
            try:
                if attempt > 0:
                    ...
                    if retry_config.strategy == RetryStrategy.EXPONENTIAL_BACKOFF:
                        ...
                        delay = min(
                            retry_config.base_delay * attempt, retry_config.max_delay
                        )
                    else:  # FIXED_INTERVAL
                    ...
                    logger.info(
                        f"Retrying {self.config.name} API call in {delay:.2f}s (attempt {attempt + 1}/{retry_config.max_attempts})"
                    )
                ...
                if attempt > 0:
                    logger.info(
                        f"{self.config.name} API call succeeded after {attempt + 1} attempts"
                    )
                ...
                if not self._is_retryable_error(e):
                    logger.warning(
                        f"Non-retryable error for {self.config.name}: {error_type}"
                    )
                    raise e

                logger.warning(
                    f"Attempt {attempt + 1} failed for {self.config.name}: {error_type}"
                )
        ...
        logger.error(
            f"All {retry_config.max_attempts} retry attempts exhausted for {self.config.name}"
        )
    ...
    async def health_check(self) -> bool:
        ...
        if (
            ...
            and (datetime.utcnow() - self._last_health_check).total_seconds()
            < self.config.reliability.health_check_interval
        ):
...
class OpenAIProvider(ReliableAIProvider):
    ...
    def _get_system_prompt(self, task_type: TaskType) -> str:
        ...
        return prompts.get(
            task_type, "You are an AI assistant helping with startup development."
        )
...
class AnthropicProvider(AIProviderInterface):
    ...
    def __init__(self, config: ProviderConfig):
        ...
        # Model mapping for different task types
        self.task_models = {
            TaskType.MARKET_RESEARCH: config.models.get(
                "research", "claude-3-5-sonnet-20241022"
            ),
            TaskType.FOUNDER_ANALYSIS: config.models.get(
                "analysis", "claude-3-5-sonnet-20241022"
            ),
            TaskType.MVP_SPECIFICATION: config.models.get(
                "specification", "claude-3-5-sonnet-20241022"
            ),
            TaskType.ARCHITECTURE_DESIGN: config.models.get(
                "architecture", "claude-3-5-sonnet-20241022"
            ),
            TaskType.CODE_GENERATION: config.models.get(
                "code", "claude-3-5-sonnet-20241022"
            ),
            TaskType.TESTING: config.models.get(
                "testing", "claude-3-5-sonnet-20241022"
            ),
            TaskType.DEPLOYMENT: config.models.get(
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_latency_ms": sum(call.latency_ms for call in self.call_history)
            / total_calls,
            "total_cost": sum(call.cost for call in self.call_history),
            "avg_tokens": sum(
                call.input_tokens + call.output_tokens for call in self.call_history
            )
            / total_calls,
        }
...
class OpenCodeCLIProvider(AIProviderInterface):
    ...
    def __init__(self, config: ProviderConfig):
        self.config = config
        self.call_history: List[ProviderCall] = []
        self.cli_path = config.models.get(
        ...
        try:
            ...
            try:
                ...
                cmd = [
                    ...
                    prompt_file,
                    "--model",
                    self.get_model_for_task(task.type),
                    "--max-tokens",
                    str(min(task.max_tokens, self.config.max_tokens)),
                    "--json",
                ]
                ...
                if result.returncode == 0:
                    ...
                    raise Exception(
                        f"CLI failed with code {result.returncode}: {stderr.decode()}"
                    )
...
class AIProviderManager:
    ...
    def register_provider(
        self, provider_name: str, provider: AIProviderInterface, config: ProviderConfig
    ):
    ...
    async def process_task(
        self, task: Task, preferred_provider: str = None
    ) -> TaskResult:
        ...
        if (
            ...
            or not self.configurations.get(
                provider_name, ProviderConfig("", "", {}, 0, 0, 0, 0)
            ).enabled
        ):
    ...
    def get_cost_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "average_latency_ms": sum(call.latency_ms for call in self.all_calls)
            / max(1, len(self.all_calls)),
            "providers": {},
        }
        ...
        for provider_name in self.providers.keys():
            provider_calls = [
                call for call in self.all_calls if call.provider == provider_name
            ]
            if provider_calls:
                stats["providers"][provider_name] = {
                    "calls": len(provider_calls),
                    "cost": sum(call.cost for call in provider_calls),
                    "success_rate": sum(1 for call in provider_calls if call.success)
                    / len(provider_calls),
                    "average_latency_ms": sum(
                        call.latency_ms for call in provider_calls
                    )
                    / len(provider_calls),
    ...
    def get_error_category_counts(self) -> Dict[str, int]:
        """Aggregate error categories from recorded calls."""
        counters: Dict[str, int] = {}
        for call in self.all_calls:
            if not call.success and call.error:
                cat, _ = classify_provider_error(Exception(call.error))
