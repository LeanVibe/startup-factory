...
from budget_monitor import BudgetMonitor, get_global_budget_monitor, BudgetExceededError
...
class ProviderCoordinator:
    ...
    def __init__(self, provider: str, provider_manager: AIProviderManager, budget_monitor: BudgetMonitor):
    ...
    async def execute_task(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if not await self.budget_monitor.can_proceed_with_task(task.startup_id, estimated_cost):
                raise BudgetExceededError(f"Task {task.id} would exceed budget limits")
            
            # Call real AI provider
            result = await self.provider_manager.call_provider(self.provider, task)
            
            # Record spending if successful
            if result.success and result.cost > 0:
                await self.budget_monitor.record_spending(
            ...
            logger.error(f"Provider {self.provider} failed for task {task.id}: {e}")
            
            # Record failed spending attempt
            await self.budget_monitor.record_spending(
                startup_id=task.startup_id,
                provider=self.provider,
                task_id=task.id,
    ...
    async def _estimate_task_cost(self, task: Task) -> float:
        """
        Estimate the cost of a task based on type and content
        ...
        """
        # Rough cost estimation based on task type and prompt length
        base_costs = {
            TaskType.MARKET_RESEARCH: 0.08,
            TaskType.FOUNDER_ANALYSIS: 0.12,
            TaskType.MVP_SPECIFICATION: 0.15,
            TaskType.ARCHITECTURE_DESIGN: 0.20,
            TaskType.CODE_GENERATION: 0.25,
            TaskType.TESTING: 0.10,
            TaskType.DEPLOYMENT: 0.15
        }
        
        base_cost = base_costs.get(task.type, 0.10)
        
        # Adjust based on prompt length (rough token estimation)
        prompt_length = len(task.prompt) + len(task.description)
...
class QueueProcessor(IQueueProcessor):
    ...
    def __init__(self, max_concurrent: int = 15, provider_manager: Optional[AIProviderManager] = None, budget_monitor: Optional[BudgetMonitor] = None):
        """
        Initialize queue processor
        
        Args:
            max_concurrent: Maximum concurrent tasks
            provider_manager: AI provider manager (optional, will create default if not provided)
            budget_monitor: Budget monitor (optional, will use global if not provided)
        """
        ...
        self.provider_manager = provider_manager or create_default_provider_manager()
        self.budget_monitor = budget_monitor or get_global_budget_monitor()
        
        # Provider coordination
        self.load_balancer = LoadBalancer()
        
        # Create coordinators for available providers
        self.provider_coordinators = {}
        for provider_name in self.provider_manager.get_available_providers():
            self.provider_coordinators[provider_name] = ProviderCoordinator(
                provider_name, self.provider_manager, self.budget_monitor
            )
        
        # Fallback coordinators for compatibility
        if not self.provider_coordinators:
            # If no real providers available, create mock coordinators
            for provider in ['openai', 'anthropic', 'perplexity']:
                self.provider_coordinators[provider] = ProviderCoordinator(
                    provider, self.provider_manager, self.budget_monitor
                )
        ...
        logger.info(f"QueueProcessor initialized with max_concurrent={max_concurrent}, providers={list(self.provider_coordinators.keys())}")
    ...
    async def get_queue_stats(self) -> dict:
        ...
        global_budget_status = await self.budget_monitor.get_global_budget_status()
        
        # Get provider cost statistics
        provider_cost_stats = self.provider_manager.get_cost_statistics()
        ...
        return {
            ...
            "budget": global_budget_status,
            "real_provider_costs": provider_cost_stats
        }
