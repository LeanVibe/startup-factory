...
Main agent-led orchestration: StartupManager, ResourceAllocator, and QueueProcessor for concurrent startup development.
...
class MultiStartupOrchestrator:
    ...
    def __init__(
        self, config_path: str = "./config.yaml", max_concurrent_startups: int = 5
    ):
        """
        ...
        logger.info(
            f"MultiStartupOrchestrator initialized (max concurrent: {max_concurrent_startups})"
        )
    ...
    async def create_startup(self, startup_config: dict) -> str:
        ...
        try:
            ...
            config = StartupConfig(
                ...
                resource_requirements=startup_config.get(
    ...
    async def create_multiple_startups(self, startup_configs: List[dict]) -> List[str]:
        ...
        logger.info(
            f"Successfully created {len(successful_ids)}/{len(startup_configs)} startups"
        )
    ...
    async def _execute_startup_workflow(self, startup_id: str) -> None:
        ...
        try:
            ...
            await self.startup_manager.update_startup_status(
                startup_id, StartupStatus.ACTIVE
            )
            ...
            for i, phase in enumerate(workflow["phases"]):
                try:
                    ...
                    await self.startup_manager.update_startup_state(
                        startup_id, {"current_phase": i, "current_phase_name": phase}
                    )

                    logger.info(
                        f"Executing phase {i}: {phase} for startup {startup_id}"
                    )

                    # Execute phase
                    phase_result = await self._execute_phase(
                        startup_id, phase, startup.config
                    )
                    ...
                    await self.startup_manager.increment_error_count(
                        startup_id, f"Phase {phase} failed: {e}"
                    )
            ...
            if workflow["status"] != "failed":
                ...
                # Update startup status
                await self.startup_manager.update_startup_status(
                    startup_id, StartupStatus.COMPLETED
                )
                ...
                total_cost = sum(
                    result.get("cost", 0.0)
                    for result in workflow["phase_results"].values()
                )
                ...
                logger.info(
                    f"Completed startup workflow for {startup_id} "
                ...
                # Mark as failed
                await self.startup_manager.update_startup_status(
                    startup_id, StartupStatus.FAILED
                )
            ...
            try:
                ...
                await self.startup_manager.increment_error_count(
                    startup_id, f"Workflow execution failed: {e}"
                )
            except Exception as nested_error:
                logger.error(f"Failed to update status after error: {nested_error}")

    async def _execute_phase(
        self, startup_id: str, phase: str, config: StartupConfig
    ) -> dict:
        """
...
""",
    ...
    def _update_success_rate(self) -> None:
        ...
        if total_attempted > 0:
            self.metrics["success_rate"] = (
                self.metrics["total_startups_completed"] / total_attempted
            )
    ...
    def _update_average_completion_time(self, completion_time: float) -> None:
        ...
        if completed_count > 0:
            ...
            self.metrics["average_completion_time"] = (
                old_avg * (completed_count - 1) + completion_time
            ) / completed_count
    ...
    async def get_startup_status(self, startup_id: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                "current_phase_name": workflow.get("phases", [])[
                    workflow.get("current_phase", 0)
                ]
            ...
            return {"startup_id": startup_id, "error": str(e)}
    ...
    async def get_all_startups_status(self) -> List[dict]:
        ...
        valid_statuses = [
            status for status in statuses if not isinstance(status, Exception)
        ]
        ...
        try:
            ...
            return {
                ...
                "current_load": {
                    ...
                    "utilization_percent": (
                        len(active_startups) / self.max_concurrent_startups
                    )
                    * 100,
                },
                "workflow_summary": {
                    ...
                    "completed_workflows": len(
                        [
                            w
                            for w in self.startup_workflows.values()
    ...
    async def health_check(self) -> dict:
        ...
        try:
            ...
            if not startup_manager_health["healthy"]:
                issues.extend(
                    startup_manager_health.get("issues", ["Startup manager unhealthy"])
                )
            if not queue_processor_health["healthy"]:
                issues.extend(
                    queue_processor_health.get("issues", ["Queue processor unhealthy"])
                )
...
async def create_demo_startup(
    orchestrator: MultiStartupOrchestrator, name: str = "Demo Startup"
) -> str:
...
async def create_multiple_demo_startups(
    orchestrator: MultiStartupOrchestrator, count: int = 3
) -> List[str]:
        ...
        try:
            ...
            for _ in range(30):  # Monitor for 30 seconds
                ...
                for status in statuses:
                    print(
                        f"  {status['name']}: {status['workflow_status']} "
                        f"(phase {status['current_phase']}/{status['total_phases']})"
                    )

                # Check if all completed
                if all(
                    s["workflow_status"] in ["completed", "failed"] for s in statuses
                ):
                    break
            ...
            print(f"\nFinal metrics:")
            print(
                f"  Total startups: {metrics['orchestrator_metrics']['total_startups_created']}"
            )
            print(
                f"  Completed: {metrics['orchestrator_metrics']['total_startups_completed']}"
            )
            print(
                f"  Success rate: {metrics['orchestrator_metrics']['success_rate']:.2%}"
            )
