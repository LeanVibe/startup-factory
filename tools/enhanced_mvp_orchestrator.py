...
A comprehensive Python script with fallback scenarios for missing API keys
...
class EnhancedAPIManager:
    ...
    async def call_openai_api(self, prompt: str, max_tokens: int = 4000) -> Tuple[str, float]:
    ...
    async def call_openai_cli_fallback(self, prompt: str) -> Tuple[str, float]:
        ...
        if not self.check_cli_availability('opencode'):
            raise Exception("OpenCode CLI not available. Please install or provide OpenAI API key.")
        ...
        try:
            ...
            cmd = ['opencode', '--non-interactive', '--input', prompt_file, '--output', 'text']
            ...
            cost = estimated_tokens * (self.providers[AIProvider.OPENAI].cost_per_1k_input / 1000)
    ...
    async def call_anthropic_api(self, prompt: str, max_tokens: int = 4000) -> Tuple[str, float]:
    ...
    async def call_provider(self, provider: AIProvider, prompt: str, max_tokens: int = 4000) -> Tuple[str, float]:
        ...
        try:
            ...
            if provider_config.available and provider_config.primary_mode == FallbackMode.API:
    ...
    def calculate_openai_cost(self, usage) -> float:
        ...
        input_cost = usage.prompt_tokens * (self.providers[AIProvider.OPENAI].cost_per_1k_input / 1000)
        output_cost = usage.completion_tokens * (self.providers[AIProvider.OPENAI].cost_per_1k_output / 1000)
        return input_cost + output_cost
    
    def calculate_anthropic_cost(self, usage) -> float:
        """Calculate Anthropic API cost"""
        input_cost = usage.input_tokens * (self.providers[AIProvider.ANTHROPIC].cost_per_1k_input / 1000)
        output_cost = usage.output_tokens * (self.providers[AIProvider.ANTHROPIC].cost_per_1k_output / 1000)
        return input_cost + output_cost
...
Format: Structured markdown with clear sections and bullet points"""
        ...
        try:
            result, cost = await self.api_manager.call_provider(AIProvider.PERPLEXITY, prompt)
            ...
            return {"content": f"Market research failed: {str(e)}", "cost": 0.0, "provider": "error"}
    
    async def run_founder_analysis(self, market_data: str, founder_profile: Dict[str, str]) -> Dict[str, Any]:
...
Output format: Scoring matrix followed by detailed recommendations
        ...
        try:
            result, cost = await self.api_manager.call_provider(AIProvider.ANTHROPIC, prompt)
            ...
            return {"content": f"Founder analysis failed: {str(e)}", "cost": 0.0, "provider": "error"}
    
    async def generate_mvp_spec(self, problem: str, solution: str, target_users: str) -> Dict[str, Any]:
...
Format: Structured markdown with clear sections
        ...
        try:
            result, cost = await self.api_manager.call_provider(AIProvider.OPENAI, prompt)
            ...
            return {"content": f"MVP specification failed: {str(e)}", "cost": 0.0, "provider": "error"}
    ...
    def display_provider_status(self):
        ...
        for provider, config in self.api_manager.providers.items():
            ...
            cli_status = "✅" if self.api_manager.check_cli_availability(config.cli_command) else "❌"
