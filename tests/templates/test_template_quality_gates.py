...
Comprehensive validation framework for Startup Factory cookiecutter templates.
Validates generated startup projects to ensure they meet production quality standards.

Features:
- Structure validation (required files/directories)
- Configuration validation (JSON/YAML/TOML parsing)
- Code quality validation (linting, syntax)
- Functionality validation (builds, tests, APIs)
- Security validation (secrets, vulnerabilities)
- Performance benchmarks (generation time, resource usage)
"""
...
class TemplateTestSuite:
    ...
    build_commands: Dict[str, List[str]]  # service -> commands
    test_commands: Dict[str, List[str]]   # service -> commands
            ...
            try:
                ...
                for pattern in dangerous_patterns:
                    if re.search(pattern, content):
                        security_issues.append(
                            f"Potentially dangerous code pattern in {py_file.relative_to(project_dir)}: {pattern}"
                        )
...
class TestTemplateQualityGates:
    """Test suite for template quality gates system"""
    
    @pytest.fixture
    def quality_gates(self):
        """Quality gates system instance"""
        return TemplateQualityGates()
    ...
    @pytest.mark.asyncio
    async def test_neoforge_template_validation(self, quality_gates, templates_dir):
        """Test neoforge template validation"""
        neoforge_path = templates_dir / "neoforge"
        
        if not neoforge_path.exists():
            pytest.skip("neoforge template not found")
        
        result = await quality_gates.validate_template(neoforge_path)
        ...
        # Quality gate: Score should be at least 0.7
        assert result.score >= 0.7, f"Template quality score too low: {result.score}"
    
    @pytest.mark.asyncio
    async def test_all_templates_validation(self, quality_gates, templates_dir):
        """Test validation of all available templates"""
        if not templates_dir.exists():
            pytest.skip("Templates directory not found")
        
        # Set the templates directory
        quality_gates.templates_dir = templates_dir
        
        results = await quality_gates.validate_all_templates()
        
        # Assertions
        assert isinstance(results, dict)
        assert len(results) > 0, "No templates found to validate"
        
        # All templates should have valid results
        for template_name, result in results.items():
            assert isinstance(result, ValidationResult)
            assert result.execution_time > 0
        ...
        # Quality gate: At least one template should pass
        passed_templates = [name for name, result in results.items() if result.is_valid]
        assert len(passed_templates) > 0, "No templates passed validation"
    
    def test_required_structure_config(self, quality_gates):
        """Test required structure configuration"""
        neoforge_structure = quality_gates._get_required_structure("neoforge")
        
        assert "files" in neoforge_structure
        assert "directories" in neoforge_structure
        assert len(neoforge_structure["files"]) > 0
        assert len(neoforge_structure["directories"]) > 0
        
        # Check for essential files
        essential_files = ["README.md", "Makefile", "backend/requirements.txt", "frontend/package.json"]
        for file_path in essential_files:
            assert file_path in neoforge_structure["files"], f"Missing essential file: {file_path}"
    
    def test_test_context_generation(self, quality_gates):
        """Test test context generation"""
        config = {
        ...
        contexts = quality_gates._generate_test_contexts(config)
        ...
        assert context["license"] == "MIT"  # First option from list
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(f"\nReport saved to: template_validation_report.md")
